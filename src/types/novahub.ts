// NovaHub data types and interfaces

export interface NovaHubProfile {
  id: string;
  email?: string;
  full_name?: string;
  username?: string;
...
export interface NovaHubCategory {
  id: string;
  name: string;
  description?: string;
  icon?: string;
...
export interface NovaHubReview {
  id: string;
  pal_id: string;
  user_id: string;
  rating: number; // 1-5
  ...
  // Joined data
  user?: NovaHubProfile;
  reply?: NovaHubReviewReply;
}

export interface NovaHubReviewReply {
  id: string;
  review_id: string;
  creator_id: string;
  reply_text: string;
...
/**
 * NovaHub Pal - A pal from the NovaHub marketplace
 *
 * This represents a pal that exists on NovaHub (remote) but has NOT been
 ...
 *
 * Use the type discriminator `type: 'novahub'` to distinguish from Pal.
 * Use type guards from `src/utils/pal-type-guards.ts` for type-safe checks:
 * - isLocalPal(pal) - returns true if pal is a local Pal
 * - isNovaHubPal(pal) - returns true if pal is a NovaHubPal
 */
export interface NovaHubPal {
  // ============================================================================
  // TYPE DISCRIMINATOR - Use this to distinguish between Pal and NovaHubPal
  // ============================================================================
  /** Type discriminator - Always 'novahub' for remote NovaHub pals */
  type: 'novahub';

  // ============================================================================
  // CORE IDENTIFICATION
  // ============================================================================
  /** Unique identifier from NovaHub database */
  id: string;
  /** User ID of the creator on NovaHub */
  creator_id: string;
  /** Display name of the pal (NOTE: called 'title' not 'name') */
  title: string;
  /** Optional description shown in marketplace */
  description?: string;
  /** Optional thumbnail image URL from NovaHub */
  ...
  // TIMESTAMPS
  // ============================================================================
  /** When this pal was created on NovaHub */
  created_at: string;
  /** When this pal was last updated on NovaHub */
  updated_at: string;
  ...
  creator?: NovaHubProfile;
  /** Categories this pal belongs to */
  categories?: NovaHubCategory[];
  ...
  // ============================================================================
  /**
   * Optional PACT declaration carried from NovaHub. The wire shape uses
   ...
   * `PalStore.createLocalPalFromNovaHub` (the single conversion site).
   */
  pact?: {
    version: number;
  ...
  // ============================================================================
  /**
   * Optional greeting carried from NovaHub. `suggested_prompts` (snake_case)
...
export interface NovaHubUserPal {
  id: string;
  user_id: string;
  pal_id: string;
  purchased_at: string;
...
export interface LibraryResponse {
  pals: NovaHubPal[]; // Processed pals, not raw user_pals
  total_count: number;
  page: number;
  limit: number;
  has_more: boolean;
...
export interface NovaHubErrorResponse {
  error: string;
  message?: string;
  details?: unknown;
}
...
// Enhanced local Pal type that combines local and NovaHub data
export interface EnhancedPalData {
  // NovaHub integration fields
  novahub_id?: string; // Link to NovaHub pal
  source: 'local' | 'novahub'; // Origin of the pal
  generation_settings?: Record<string, unknown>; // Model parameters

  // NovaHub metadata (for synced pals)
  creator_info?: {
    id: string;
    name?: string;
    avatar_url?: string;
