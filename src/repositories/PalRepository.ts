...
class PalRepository {
        ...
        for (const legacyPal of legacyPals) {
          ...
          await database.collections
            ...
            .create((record: LocalPal) => {
              ...
              record.novahubId = pal.novahub_id;
              ...
              record.generationSettings = LocalPal.safeStringify(
                pal.rawNovahubGenerationSettings,
              );
            });
        }
      });
      ...
      const newPal = await database.write(async () => {
        return await database.collections
          ...
          .create((record: LocalPal) => {
            ...
            record.novahubId = palData.novahub_id;
            ...
            // Save generation settings (prefer local over NovaHub)
            const generationSettings =
              palData.completionSettings ||
              palData.rawNovahubGenerationSettings;
    ...
    try {
      const updatedPal = await database.write(async () => {
        ...
        return await localPal.update((record: LocalPal) => {
          ...
          if (updates.novahub_id !== undefined) {
            record.novahubId = updates.novahub_id;
          }
          ...
          if (
            updates.rawNovahubGenerationSettings !== undefined ||
            updates.completionSettings !== undefined
          ) {
            // Update generation settings (prefer local over NovaHub)
            record.generationSettings = LocalPal.safeStringify(
              updates.completionSettings ||
                updates.rawNovahubGenerationSettings,
            );
          }
  ...
  async getNovaHubPals(): Promise<Pal[]> {
    try {
      const novahubPals = await database.collections
        .get<LocalPal>('local_pals')
        .query(Q.where('source', 'novahub'))
        .fetch();

      return novahubPals.map(pal => pal.toPal());
    } catch (error) {
      console.error('Error fetching novahub pals:', error);
      return [];
    }
  }
