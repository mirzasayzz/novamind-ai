...
import {AuthSheet, PalDetailSheet} from '../../components/NovaHub';
...
export const PalsScreen: React.FC = observer(() => {
  ...
  const [searchResults, setSearchResults] = useState<NovaHubPal[]>([]);

  // Sheet states
  const [showProfile, setShowProfile] = useState(false);
  const [showAuth, setShowAuth] = useState(false);
  const [showPalDetail, setShowPalDetail] = useState(false);
  const [selectedPal, setSelectedPal] = useState<NovaHubPal | null>(null);
    ...
    const runInitialSetup = async () => {
      try {
        ...
        if (authService.isAuthenticated) {
          ...
          if (needsSync) {
            console.log('Syncing with NovaHub...');
  ...
  const loadData = async () => {
    try {
      ...
      await palStore.searchNovaHubPals({sortBy: 'newest', limit: 20});
      if (authService.isAuthenticated) {
        await Promise.all([
  ...
  const handlePalPress = (pal: NovaHubPal | Pal) => {
    handlePalByType(pal, {
      onLocalPal: localPal => {
        // Local pal - handle edit
        handleEditPal(localPal);
      },
      onNovaHubPal: novaHubPal => {
        // NovaHub pal - show detail sheet
        setSelectedPal(novaHubPal);
  ...
  const getFilteredData = (): (NovaHubPal | Pal)[] => {
    if (isSearchExpanded && searchResults.length > 0) {
      return searchResults;
    }

    const localPals = palStore.getLocalPals();
    const downloadedPals = palStore.getDownloadedNovaHubPals();
    const hubPals = palStore.cachedNovaHubPals;
  ...
  const getSectionedData = (): Array<{
    ...
    switch (activeFilter) {
      case 'all': {
        const sections: Array<{title: string; data: (NovaHubPal | Pal)[]}> = [];
