...
interface ExpandableSearchProps {
  isExpanded: boolean;
  onToggle: () => void;
  onSearchResults: (results: NovaHubPal[]) => void;
}
...
export const ExpandableSearch: React.FC<ExpandableSearchProps> = observer(
  ({isExpanded, onToggle, onSearchResults}) => {
    ...
    const performSearch = useCallback(async () => {
      ...
      try {
        await palStore.searchNovaHubPals({
          query: searchQuery,
        });
        // For now, use cached results since searchNovaHubPals updates the store
        onSearchResults(palStore.cachedNovaHubPals);
      } catch (error) {
