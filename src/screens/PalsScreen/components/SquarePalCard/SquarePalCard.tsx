...
import {getPalDisplayLabel} from '../../../../utils/novahub-display';
import {hasVideoCapability} from '../../../../utils/pal-capabilities';
import {isLocalPal, isNovaHubPal} from '../../../../utils/pal-type-guards';
...
const getDisplayContent = (pal: NovaHubPal | Pal): string => {
  // Priority 1: NovaHub description
  if (pal.description) {
  ...
  const systemPrompt = isNovaHubPal(pal) ? pal.system_prompt : pal.systemPrompt;
...
const PalThumbnail: React.FC<{
  ...
  const palName = isNovaHubPal(pal) ? pal.title : pal.name;
  ...
  return (
    <View style={thumbnailStyle}>
      ...
      {(isLocal ||
        (isNovaHubPal(pal) && palStore.isNovaHubPalDownloaded(pal.id))) && (
      ...
      {/* Badges */}
      {isNovaHubPal(pal) && pal.protection_level === 'reveal_on_purchase' && (
...
export const SquarePalCard: React.FC<SquarePalCardProps> = observer(
  ({pal, onPress, isLocal = false}) => {
    ...
    const handleStartChat = async () => {
      try {
        // Handle NovaHub pals that need to be downloaded first
        let localPal: Pal | undefined;

        if (isNovaHubPal(pal)) {
          // Check if this NovaHub pal is already downloaded
          localPal = palStore.pals.find(p => p.novahub_id === pal.id);
              ...
              [
                {text: 'Cancel', style: 'cancel'},
                {
                  ...
                  onPress: async () => {
                    try {
                      const downloadedPal =
                        await palStore.downloadNovaHubPal(pal);
    ...
    const getDisplayLabel = (pal_: NovaHubPal) => {
      return getPalDisplayLabel(pal_);
    };

    const palColors = isLocalPal(pal) ? pal.color : null;
    const palName = isNovaHubPal(pal) ? pal.title : pal.name;
    const palLabel = isNovaHubPal(pal) ? getDisplayLabel(pal) : null;
    const palRating = isNovaHubPal(pal) ? pal.average_rating : pal.rating;
    const palReviewCount = isNovaHubPal(pal)
      ? pal.review_count
      : pal.review_count;
    const palTags = isNovaHubPal(pal) ? pal.tags : undefined;
    const palCreator = isNovaHubPal(pal) ? pal.creator : undefined;
    const isProtected =
      isNovaHubPal(pal) && pal.protection_level === 'reveal_on_purchase';
    ...
    return (
      <View style={styles.cardOuter}>
        <TouchableOpacity
          testID={`${isNovaHubPal(pal) ? 'novahub' : 'local'}-pal-card-${pal.id}`}
