...
describe('SquarePalCard', () => {
  ...
  // Create a NovaHub pal fixture
  const createNovaHubPal = (
    overrides: Partial<NovaHubPal> = {},
  ): NovaHubPal => ({
    type: 'novahub',
    id: 'ph-pal-1',
  ...
  describe('Rendering', () => {
    ...
    it('renders NovaHub pal correctly', () => {
      const pal = createNovaHubPal();
      const {getByText} = render(
    ...
    it('renders rating and review count for NovaHub pal', () => {
      const pal = createNovaHubPal({average_rating: 4.5, review_count: 10});
      const {getByText} = render(
        <SquarePalCard pal={pal} onPress={mockOnPress} />,
      );
    ...
    it('renders tags for NovaHub pal', () => {
      const pal = createNovaHubPal();
      const {getByText} = render(
        <SquarePalCard pal={pal} onPress={mockOnPress} />,
      );
    ...
    it('renders protection badge for protected NovaHub pals', () => {
      const pal = createNovaHubPal({protection_level: 'reveal_on_purchase'});
  ...
  describe('Chat Navigation', () => {
    ...
    it('shows download alert for NovaHub pal not yet downloaded', async () => {
      jest.spyOn(Alert, 'alert');
      const pal = createNovaHubPal();
      ...
      // NovaHub pals that aren't downloaded shouldn't show chat button
      const touchables = UNSAFE_getAllByType(
        require('react-native').TouchableOpacity,
      );
  ...
  describe('Model Warning', () => {
    ...
    it('does not show model warning for NovaHub pals', () => {
      const pal = createNovaHubPal();
      const {queryByText} = render(
        <SquarePalCard pal={pal} onPress={mockOnPress} />,
      );
  ...
  describe('Action Buttons Visibility', () => {
    ...
    it('does not show share and delete buttons for NovaHub pals', () => {
      const pal = createNovaHubPal();
      const {UNSAFE_queryAllByType} = render(
        <SquarePalCard pal={pal} onPress={mockOnPress} />,
      );

      // NovaHub pals should not have action buttons in header
    ...
    it('shows chat button for downloaded NovaHub pal', () => {
      const novaHubPal = createNovaHubPal();
      const localPal = createLocalPal({novahub_id: novaHubPal.id});
      palStore.pals = [localPal];
      palStore.isNovaHubPalDownloaded = jest.fn().mockReturnValue(true);

      const {UNSAFE_getAllByType} = render(
        <SquarePalCard pal={novaHubPal} onPress={mockOnPress} />,
      );
  ...
  describe('Premium Badge', () => {
    it('shows premium badge for premium NovaHub pals', () => {
      const pal = createNovaHubPal({
        price_cents: 999,
      });
      ...
      const card = getByText('NovaHub Test Pal');
