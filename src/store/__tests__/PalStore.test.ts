...
describe('PalStore', () => {
  ...
  beforeEach(() => {
    ...
    runInAction(() => {
      palStore.pals = [];
      palStore.cachedNovaHubPals = [];
      palStore.userLibrary = [];
      palStore.userCreatedPals = [];
      palStore.isLoadingNovaHub = false;
  ...
  describe('NovaHub Integration', () => {
    describe('searchNovaHubPals', () => {
      it('should search pals and update state', async () => {
        ...
        (novaHubService.getPals as jest.Mock).mockResolvedValue(mockResponse);

        expect(palStore.isLoadingNovaHub).toBe(false);
        expect(palStore.syncState.status).toBe('idle');

        const result = await palStore.searchNovaHubPals({query: 'test'});

        expect(novaHubService.getPals).toHaveBeenCalledWith({query: 'test'});
        expect(result).toEqual(mockResponse);
        expect(palStore.cachedNovaHubPals).toEqual(mockResponse.pals);
        expect(palStore.isLoadingNovaHub).toBe(false);
      ...
      it('should handle search errors gracefully', async () => {
        ...
        (novaHubService.getPals as jest.Mock).mockRejectedValue(error);

        const consoleSpy = jest.spyOn(console, 'warn').mockImplementation();

        // Should not throw, but return empty results
        const result = await palStore.searchNovaHubPals();
        ...
        expect(palStore.cachedNovaHubPals).toEqual([]);
        expect(palStore.isLoadingNovaHub).toBe(false);
    ...
    describe('loadUserLibrary', () => {
      it('should load user library and update state', async () => {
        ...
        (novaHubService.getLibrary as jest.Mock).mockResolvedValue(
          mockResponse,
        );

        const result = await palStore.loadUserLibrary();

        expect(novaHubService.getLibrary).toHaveBeenCalled();
        expect(result).toEqual(mockResponse);
      ...
      it('should handle library loading errors gracefully', async () => {
        ...
        (novaHubService.getLibrary as jest.Mock).mockRejectedValue(error);
    ...
    describe('helper methods', () => {
      it('should get categories', async () => {
        ...
        (novaHubService.getCategories as jest.Mock).mockResolvedValue(
          mockCategories,
        );
        ...
        expect(novaHubService.getCategories).toHaveBeenCalled();
      });
      ...
      it('should get tags', async () => {
        ...
        (novaHubService.getTags as jest.Mock).mockResolvedValue(mockTags);

        const result = await palStore.getTags({query: 'help'});

        expect(result).toEqual(mockTags);
        expect(novaHubService.getTags).toHaveBeenCalledWith({query: 'help'});
      });

      it('should get specific pal', async () => {
        const mockNovaHubPalResponse = {
          ...mockNovaHubPal,
          id: '1',
          title: 'Test Pal',
          creator_id: 'user1',
        ...
        (novaHubService.getPal as jest.Mock).mockResolvedValue(
          mockNovaHubPalResponse,
        );

        const result = await palStore.getNovaHubPal('1');

        expect(result).toEqual(mockNovaHubPalResponse);
        expect(novaHubService.getPal).toHaveBeenCalledWith('1');
      });

      it('should check pal ownership', async () => {
        const mockOwnership = {owned: true, purchase_date: '2023-01-01'};

        (novaHubService.checkPalOwnership as jest.Mock).mockResolvedValue(
          mockOwnership,
        );
        ...
        expect(novaHubService.checkPalOwnership).toHaveBeenCalledWith('pal-id');
      });
    });

    describe('downloadNovaHubPal', () => {
      it('should download pal with provided information', async () => {
        const palToDownload: NovaHubPal = {
          ...mockNovaHubPal,
          id: 'pal-to-download',
          title: 'Test Pal',
          system_prompt:
        ...
        const expectedLocalPal = {
          ...
          systemPrompt:
            'You are a {{role}} assistant with {{expertise}} knowledge.',
          source: 'novahub',
          novahub_id: 'pal-to-download',
          rawNovahubGenerationSettings: palToDownload.model_settings,
        };
        ...
        const result = await palStore.downloadNovaHubPal(palToDownload);
      ...
      it('should handle premium pal ownership check before downloading', async () => {
        ...
        await palStore.downloadNovaHubPal(premiumPal);

        // Verify ownership was checked
        expect(novaHubService.checkPalOwnership).toHaveBeenCalledWith(
          'premium-pal-id',
        );
      });
      ...
      it('should reject download for unowned premium pal', async () => {
        ...
        await expect(palStore.downloadNovaHubPal(premiumPal)).rejects.toThrow(
    ...
    describe('createLocalPalFromNovaHub (via downloadNovaHubPal)', () => {
      ...
      const buildNovaHubPal = (overrides: Partial<NovaHubPal>): NovaHubPal => ({
        ...mockNovaHubPal,
        id: 'conversion-test',
        ...overrides,
      });
      ...
      it('happy path: maps pact + greeting with snake_case to camelCase rename', async () => {
        const novaHubPal = buildNovaHubPal({
          pact: {
            version: 1,
            talents: [
        ...
        await palStore.downloadNovaHubPal(novaHubPal);
      ...
      it('legacy / both absent: pact and greeting are undefined', async () => {
        const novaHubPal = buildNovaHubPal({});
        // mockNovaHubPal has no pact / greeting — older Novahub server shape.
  ...
  describe('Utility Methods', () => {
    ...
    it('should check if NovaHub pal is downloaded', () => {
      expect(palStore.isNovaHubPalDownloaded('ph-123')).toBe(true);
      expect(palStore.isNovaHubPalDownloaded('ph-456')).toBe(false);
    });
  });
});
