...
jest.mock('../../services/novahub/NovaHubApiService', () => ({
  novaHubApiService: {createCheckoutSession: jest.fn()},
}));
jest.mock('../../services', () => ({
  novaHubService: {checkPalOwnership: jest.fn()},
}));
...
import {novaHubApiService} from '../../services/novahub/NovaHubApiService';
...
const createSession = novaHubApiService.createCheckoutSession as jest.Mock;
const checkPalOwnership = novaHubService.checkPalOwnership as jest.Mock;
...
describe('CheckoutFlowStore', () => {
  ...
  it('200 -> linking -> launched -> browser_open and opens the auth session', async () => {
    ...
    expect(openAuth).toHaveBeenCalledWith(session.checkout_url, 'novamind');
  ...
  it('a stale same-pal callback does not mutate a newer flow', async () => {
    ...
    resolveOld('novamind://checkout/cancel');
    await flushMicrotasks();
    expect(checkoutFlowStore.status).toBe('browser_open');
  });
  ...
  describe('Android link-out prep (linking status)', () => {
    ...
    it("'launched' without a token -> opens the tab; later owned does not report", async () => {
      ...
      openAuth.mockResolvedValueOnce('novamind://checkout/success');
      await checkoutFlowStore.start('pal-1');
      await flushMicrotasks();
  ...
  it('openAuth resolves a success callback -> reconcile -> owned', async () => {
    openAuth.mockResolvedValue('novamind://checkout/success?purchase_id=pur_1');
    checkPalOwnership.mockResolvedValueOnce({owned: true});
    await checkoutFlowStore.start('pal-1');
    await flushMicrotasks();
    expect(checkoutFlowStore.status).toBe('finalizing');
  ...
  it('openAuth resolves a cancel callback -> cancelled, silent', async () => {
    openAuth.mockResolvedValue('novamind://checkout/cancel');
    await checkoutFlowStore.start('pal-1');
    await flushMicrotasks();
    expect(checkoutFlowStore.status).toBe('cancelled');
  });
  ...
  it('openAuth resolves an unexpected path -> cancelled, silent', async () => {
    ...
    openAuth.mockResolvedValue('novamind://checkout/unexpected');
    await checkoutFlowStore.start('pal-1');
    await flushMicrotasks();
    expect(checkoutFlowStore.status).toBe('cancelled');
  });
...
describe('CheckoutFlowStore — iOS (link-out prep absent)', () => {
  ...
  beforeEach(() => {
    ...
    jest.doMock('../../services/novahub/NovaHubApiService', () => ({
      novaHubApiService: {
  ...
  it('reconcile-success owned does not report (iOS never reports)', async () => {
    ...
    (
      iosServices.novaHubService.checkPalOwnership as jest.Mock
