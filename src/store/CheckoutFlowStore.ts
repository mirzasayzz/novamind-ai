...
import {NOVAHUB_API_BASE_URL} from '@env';
 ...
 * CheckoutFlowStore
 *
 * App-global owner of the NovaHub in-app checkout state on both platforms. Sole
...
// the checkout return under the shared novamind:// scheme.
const CALLBACK_HOST = 'checkout';

const CALLBACK_SCHEME = 'novamind';
...
// the NovaHub host is allowed too in case it serves/redirects the page itself.
const isAllowedCheckoutUrl = (value: string): boolean => {
  try {
    const url = new URL(value);
    const host = url.hostname.toLowerCase();
    const novaHubHost = new URL(NOVAHUB_API_BASE_URL).hostname.toLowerCase();
...
class CheckoutFlowStore {
  ...
  async start(palId: string): Promise<void> {
    ...
    const successUrl = `${NOVAHUB_API_BASE_URL}/app-return/checkout/success`;
    const cancelUrl = `${NOVAHUB_API_BASE_URL}/app-return/checkout/cancel`;

    try {
      const session = await novaHubApiService.createCheckoutSession(palId, {
        successUrl,
        cancelUrl,
      });
  ...
  // novamind://checkout/{success|cancel} callback. A reject (user-dismiss /
  ...
  private async reconcile(palId: string): Promise<void> {
    ...
    for (let i = 0; i < RECONCILE_BACKOFFS_MS.length; i++) {
      ...
      try {
        const {owned} = await novaHubService.checkPalOwnership(palId);
        if (this.epoch !== myEpoch) {
          return;
        }
        if (owned) {
