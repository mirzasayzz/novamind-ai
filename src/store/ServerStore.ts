...
import {AGENTROUTER_BASE_URL, AGENTROUTER_API_KEY} from '@env';
...
const KEYCHAIN_SERVICE_PREFIX = 'novamind-server-';
...
class ServerStore {
  ...
  /**
   * Seed the built-in AgentRouter cloud endpoint (OpenAI-compatible) as a
   * remote server. Runs once after store hydration; no-op when env is unset
   * or a server with the same url already exists (idempotent).
   */
  seedAgentRouterServer(): void {
    if (!AGENTROUTER_BASE_URL || !AGENTROUTER_API_KEY) {
      return;
    }
    if (this.servers.some(s => s.url === AGENTROUTER_BASE_URL)) {
      return;
    }
    const id = this.addServer({
      name: 'AgentRouter',
      url: AGENTROUTER_BASE_URL,
    });
    // The key lives in the Keychain, never in the persisted store.
    this.setApiKey(id, AGENTROUTER_API_KEY);
  }
