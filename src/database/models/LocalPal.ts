...
export default class LocalPal extends Model {
  ...
  @field('source') source!: string; // 'local' | 'novahub'
  @field('novahub_id') novahubId?: string;
  ...
  get completionSettingsObject(): CompletionParams | undefined {
    try {
      ...
      if (settings && typeof settings === 'object') {
        ...
        // This is especially important for NovaHub pals that may only have partial settings
  ...
  toPal(): Pal {
    return {
      ...
      source: this.source as 'local' | 'novahub',
      novahub_id: this.novahubId,
      ...
      rawNovahubGenerationSettings: this.generationSettingsObject,
