...
export const PalDetailSheet: React.FC<PalDetailSheetProps> = observer(
  ({pal, isVisible, onClose, onSignInPress}) => {
    ...
    const [detailedPal, setDetailedPal] = useState<NovaHubPal | null>(null);
    ...
    useEffect(() => {
      const fetchPalDetails = async () => {
        ...
        try {
          ...
          setError(null);
          const detailed = await novaHubService.getPal(pal.id);
    ...
    const isDownloaded = palStore.isNovaHubPalDownloaded(displayPal.id);
    ...
    const handleAction = async () => {
      ...
      try {
        setIsLoading(true);
        setError(null);
        await palStore.downloadNovaHubPal(palToDownload);
        Alert.alert(
            ...
            (palStore.isCheckoutEligible ? (
              <View style={styles.buyActionColumn}>
                <Button
                  ...
                  {l10n.palsScreen.palDetailSheet.buyOnNovahub}
                </Button>
