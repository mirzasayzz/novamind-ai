...
import {authService, novaHubService} from '../../../../services';
...
describe('PalDetailSheet', () => {
  ...
  beforeEach(() => {
    ...
    palStore.pals = [];
    // Reset novaHubService mocks
    (novaHubService.getPal as jest.Mock).mockResolvedValue(mockNovaHubPal);
    // Ensure isNovaHubPalDownloaded returns false by default
    (palStore.isNovaHubPalDownloaded as jest.Mock).mockImplementation(
      () => false,
    );
    // Reset downloadNovaHubPal to resolve successfully
    (palStore.downloadNovaHubPal as jest.Mock).mockResolvedValue(undefined);
  ...
  describe('Rendering', () => {
    ...
    it('displays pal title', async () => {
      ...
      await waitFor(() => {
        expect(getByText('NovaHub Test Pal')).toBeTruthy();
      });
    });
    ...
    it('displays description', async () => {
      ...
      await waitFor(() => {
        expect(getByText('A test pal from NovaHub')).toBeTruthy();
      });
    });
  ...
  describe('Fetching Pal Details', () => {
    it('fetches detailed pal information when sheet opens', async () => {
      ...
      await waitFor(() => {
        expect(novaHubService.getPal).toHaveBeenCalledWith(mockNovaHubPal.id);
      });
    });
    ...
    it('does not fetch details when pal is null', () => {
      ...
      expect(novaHubService.getPal).not.toHaveBeenCalled();
    });
    ...
    it('handles fetch error gracefully and falls back to basic pal', async () => {
      ...
      (novaHubService.getPal as jest.Mock).mockRejectedValueOnce(fetchError);
  ...
  describe('Free Pal Actions', () => {
    ...
    it('downloads pal when download button is pressed', async () => {
      ...
      await waitFor(() => {
        expect(palStore.downloadNovaHubPal).toHaveBeenCalledWith(
          mockNovaHubPal,
        );
      });
    ...
    it('shows downloaded state when pal is already downloaded', async () => {
      (palStore.isNovaHubPalDownloaded as jest.Mock).mockReturnValue(true);
    ...
    it('handles download error gracefully', async () => {
      ...
      (palStore.downloadNovaHubPal as jest.Mock).mockRejectedValue(
        downloadError,
      );
  ...
  describe('Premium Pal Display', () => {
    it('shows premium label for premium pals', async () => {
      ...
      const {getByTestId} = render(
        <PalDetailSheet {...defaultProps} pal={mockPremiumNovaHubPal} />,
      );
    ...
    it('hides system prompt for unowned premium pals', async () => {
      ...
      await waitFor(() => {
        expect(
          queryByText('You are a helpful assistant from NovaHub.'),
        ).toBeNull();
      });
    });
  ...
  describe('Owned Premium Pal Actions', () => {
    ...
    it('shows system prompt for owned premium pals', async () => {
      ...
      await waitFor(() => {
        expect(
          getByText('You are a helpful assistant from NovaHub.'),
        ).toBeTruthy();
      });
    });
  ...
  describe('Edge Cases', () => {
    it('handles pal without creator gracefully', async () => {
      const palWithoutCreator = createNovaHubPal({
        creator: undefined,
      });
      (novaHubService.getPal as jest.Mock).mockResolvedValue(palWithoutCreator);
    ...
    it('handles pal without description', async () => {
      const palWithoutDescription = createNovaHubPal({
    ...
    it('handles pal without categories', async () => {
      const palWithoutCategories = createNovaHubPal({
        categories: [],
      });
      (novaHubService.getPal as jest.Mock).mockResolvedValue(
    ...
    it('handles pal without tags', async () => {
      const palWithoutTags = createNovaHubPal({
        tags: [],
      });
      (novaHubService.getPal as jest.Mock).mockResolvedValue(palWithoutTags);
  ...
  describe('Loading States', () => {
    it('shows loading state during download', async () => {
      ...
      (palStore.downloadNovaHubPal as jest.Mock).mockReturnValue(
        downloadPromise,
      );
      ...
      await waitFor(() => {
        expect(palStore.downloadNovaHubPal).toHaveBeenCalled();
      });
  ...
  describe('Premium Buy Button (eligible vs ineligible)', () => {
    ...
    it('does not show buy button for free pals', async () => {
      ...
      const {queryByTestId} = render(
        <PalDetailSheet {...defaultProps} pal={mockNovaHubPal} />,
      );
    ...
    it('shows the finalizing indicator while the purchase settles', async () => {
      ...
      runInAction(() => {
        ...
        checkoutFlowStore.palId = mockPremiumNovaHubPal.id;
      });
