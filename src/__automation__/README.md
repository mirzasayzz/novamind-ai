...
The Android `e2e` flavor (`applicationId com.novamind.e2e`,
buildType `releaseE2e`) is the production-like build that sets
`E2E_BUILD=true`. The default prod flavor (`com.novamind`) leaves
`E2E_BUILD` unset.
...
|--------|---------|------------|
| `BenchmarkRunnerScreen` | Drives the benchmark matrix in-app for the `benchmark-matrix` spec | Deep link `novamind://e2e/benchmark` (registered in `android/app/src/e2e/AndroidManifest.xml`); manual button tap to start |
...
- `memory` — `novamind://memory?cmd=snap::<label>` etc. (memory-profile spec)
