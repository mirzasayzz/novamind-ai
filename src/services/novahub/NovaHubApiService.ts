...
// Status carried on NovaHubError.details for checkout error mapping.
// 'already_owned' marks a 400 the caller treats as success.
export type CheckoutErrorStatus = 'already_owned' | 401 | 404 | 500 | 'network';

class NovaHubApiService {
  private apiBase = NOVAHUB_API_BASE_URL;
  ...
  private async apiRequest<T>(
    ...
    if (!this.isConfigured()) {
      throw new NovaHubError(
        'NovaHub API not configured - missing NOVAHUB_API_BASE_URL',
      );
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new NovaHubError(errorMessage, {
      ...
      throw new NovaHubError(`Network error: ${errorMessage}`, error);
    }
  }

  // Transform API pal response to internal format
  private transformApiPal(apiPal: ApiPalResponse): NovaHubPal {
    return {
      type: 'novahub' as const,
      id: apiPal.id,
  ...
  async createCheckoutSession(
    ...
    try {
      ...
      if (error instanceof NovaHubError) {
        ...
        throw new NovaHubError(error.message, {status: errorStatus});
      }
      throw new NovaHubError(
  ...
  async getLibrary(query: LibraryQuery = {}): Promise<LibraryResponse> {
    ...
    if (!authService.session?.access_token) {
      throw new NovaHubError('No valid session - please sign in again');
    }
...
export const novaHubApiService = new NovaHubApiService();
