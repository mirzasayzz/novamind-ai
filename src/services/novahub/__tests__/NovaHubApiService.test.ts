// Tests for NovaHubApiService

describe('NovaHubApiService', () => {
  beforeEach(() => {
    jest.resetModules();
    jest.clearAllMocks();
    // @ts-ignore
  ...
  it('throws when NOVAHUB_API_BASE_URL is not configured', async () => {
    jest.doMock('@env', () => ({NOVAHUB_API_BASE_URL: undefined}));
    jest.doMock('../supabase', () => ({
      getAuthHeaders: jest.fn().mockResolvedValue({}),
    }));

    const {novaHubApiService, NovaHubError} = require('../NovaHubApiService');

    await expect(novaHubApiService.getPals()).rejects.toThrow(NovaHubError);
    await expect(novaHubApiService.getPals()).rejects.toThrow(
      'NovaHub API not configured',
    );
  });

  it('builds correct URL and maps response in getPals', async () => {
    jest.doMock('@env', () => ({NOVAHUB_API_BASE_URL: 'https://api.test'}));
    jest.doMock('../supabase', () => ({
      getAuthHeaders: jest.fn().mockResolvedValue({}),
    }));
    ...
    const result = await novaHubApiService.getPals({
      query: 'foo',
      category_ids: ['cat1'],
      tag_names: ['tag1'],
      price_min: 1,
  ...
  it('enforces auth guards for getLibrary', async () => {
    ...
    const api1 = require('../NovaHubApiService');
    await expect(api1.novaHubApiService.getLibrary()).rejects.toThrow(
    ...
    const api2 = require('../NovaHubApiService');
    await expect(api2.novaHubApiService.getLibrary()).rejects.toThrow(
      'No valid session',
    );
  });
  ...
  describe('createCheckoutSession', () => {
    ...
    const loadService = () => {
      ...
      return require('../NovaHubApiService');
    };

    it('posts to /api/mobile/purchases with the Bearer header and return URLs', async () => {
      const {novaHubApiService} = loadService();
      // @ts-ignore
      global.fetch = jest
        .fn()
        .mockResolvedValue({ok: true, json: async () => session});

      const result = await novaHubApiService.createCheckoutSession('pal-1', {
    ...
    const mapStatus = async (http: number, body: object) => {
      const {novaHubApiService, NovaHubError} = loadService();
      // @ts-ignore
      global.fetch = jest.fn().mockResolvedValue({
        ok: false,
        status: http,
      ...
      const err = await novaHubApiService
        .createCheckoutSession('pal-1', {
          successUrl: 'https://host.test/app-return/success',
          cancelUrl: 'https://host.test/app-return/cancel',
        })
        .catch((e: unknown) => e);
      expect(err).toBeInstanceOf(NovaHubError);
    ...
    it('maps a fetch throw to network', async () => {
      ...
      await expect(
        novaHubApiService.createCheckoutSession('pal-1', {
          successUrl: 'https://host.test/app-return/success',
          cancelUrl: 'https://host.test/app-return/cancel',
        }),
  ...
  describe('transformApiPal', () => {
    ...
    beforeEach(() => {
      jest.resetModules();
      jest.doMock('@env', () => ({
        NOVAHUB_API_BASE_URL: 'https://api.test.com',
      }));
      jest.doMock('../AuthService', () => ({
    ...
    it('should transform a complete API pal response', () => {
      ...
      expect(result.type).toBe('novahub');
    ...
    it('forwards pact, greeting, images, models when present', () => {
      ...
      // (PalStore.createLocalPalFromNovaHub).
