...
import {novaHubService} from './NovaHubService';
import {NovaHubErrorHandler, RetryHandler} from './ErrorHandler';

import type {NovaHubPal, SyncState} from '../../types/novahub';
...
class SyncService {
  ...
  async syncAll(): Promise<void> {
    ...
    try {
      ...
      const errorInfo = NovaHubErrorHandler.handle(error);
  ...
  async syncUserLibrary(): Promise<void> {
    ...
    try {
      const libraryResponse = await RetryHandler.withRetry(() =>
        novaHubService.getLibrary({limit: 20}),
      );
      ...
      await database.write(async () => {
        ...
        for (const pal of libraryResponse.pals) {
          await userLibraryCollection.create((entry: UserLibrary) => {
            ...
            entry.novahubId = pal.id; // Use pal.id instead of pal_id
      ...
      await this.updateSyncStatus(
        'library',
        'error',
        NovaHubErrorHandler.handle(error).message,
      );
      throw error;
    }
  }
    ...
    try {
      ...
      await database.write(async () => {
        ...
        const existingCategories = await globalSettingsCollection
          .query(Q.where('key', 'novahub_categories'))
          .fetch();
        ...
        if (existingCategories.length > 0) {
          ...
          await globalSettingsCollection.create((setting: any) => {
            setting.key = 'novahub_categories';
            setting.value = categoriesData;
  ...
  async syncTags(): Promise<void> {
    try {
      const tagsResponse = await RetryHandler.withRetry(() =>
        novaHubService.getTags({limit: 20}),
      );
      ...
      await database.write(async () => {
        ...
        const existingTags = await globalSettingsCollection
          .query(Q.where('key', 'novahub_tags'))
          .fetch();
  ...
  async syncCachedPalsMetadata(): Promise<void> {
    try {
      ...
      for (const cachedPal of cachedPals) {
        try {
          const updatedPal = await novaHubService.getPal(cachedPal.novahubId);
          ...
          console.warn(
            `Failed to update cached Pal ${cachedPal.novahubId}:`,
            error,
          );
        }
      }
  ...
  async cachePal(pal: NovaHubPal): Promise<void> {
    const cachedPalsCollection = database.get<CachedPal>('cached_pals');

    await database.write(async () => {
      // Check if already cached
      const existing = await cachedPalsCollection
        .query(Q.where('novahub_id', pal.id))
        .fetch();

      if (existing.length > 0) {
        // Update existing
        await existing[0].update((cachedPal: CachedPal) => {
          this.updateCachedPalFromNovaHubPal(cachedPal, pal);
        });
      } else {
        // Create new
  ...
  private updateCachedPalFromNovaHubPal(
    cachedPal: CachedPal,
    pal: NovaHubPal,
  ): void {
    cachedPal.novahubId = pal.id;
