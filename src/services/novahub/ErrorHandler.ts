...
import type {NovaHubErrorResponse} from '../../types/novahub';
...
export class NovaHubErrorHandler {
  static handle(error: unknown): ErrorInfo {
    // Handle NovaHub API errors
    if (error instanceof NovaHubError) {
      return this.handleNovaHubError(error);
    }
  ...
  private static handleNovaHubError(error: NovaHubError): ErrorInfo {
    // NovaHubError doesn't have statusCode, use simple error handling
    return {
      type: 'unknown',
      message: error.message,
      userMessage: error.message || 'An error occurred with NovaHub.',
      retryable: false,
    };
  }

  private static handleAxiosError(
    error: AxiosError<NovaHubErrorResponse>,
  ): ErrorInfo {
  ...
  static async withRetry<T>(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        lastError = NovaHubErrorHandler.handle(error);
        ...
        const delay = NovaHubErrorHandler.getRetryDelay(lastError, attempt);
        if (delay > 0) {
