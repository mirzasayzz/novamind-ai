 ...
 */
export function parseNovaHubTemplate(template: string): ParsedMustacheTemplate {
  ...
  // Convert schema to NovaMind format
  const parameterSchema = schema ? convertJsonSchemaToNovaMind(schema) : [];
...
/**
 * Converts JSON schema to NovaMind ParameterDefinition format
 */
function convertJsonSchemaToNovaMind(
  ...
  for (const [key, definition] of Object.entries(schema)) {
    ...
    // Map schema types to NovaMind types
    let novahubType: ParameterDefinition['type'];
    switch (definition.type) {
      case 'select':
        novahubType = 'select';
        break;
      case 'combobox':
        novahubType = 'combobox';
        break;
      case 'text':
      default:
        novahubType = 'text';
    ...
    if (
      (novahubType === 'select' || novahubType === 'combobox') &&
