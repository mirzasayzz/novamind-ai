 ...
 * values fill the gaps (the NovaHub flow already carries them). When omitted,
 ...
 * matching `filename`. With a `fallback` (the NovaHub flow), fetch failures and
...
export const resolveHFModelForDownload = async (
  ...
  // throws. Tolerant callers (NovaHub) keep per-fetch independence so a partial
