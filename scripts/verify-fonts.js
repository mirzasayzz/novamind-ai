 ...
 *   - <string>Name.ttf</string> inside ios/NovaMind/Info.plist (UIAppFonts)
 *
...
const INFO_PLIST = path.join(ROOT, 'ios', 'NovaMind', 'Info.plist');
...
function main() {
  ...
  for (const name of [...families].sort()) {
    ...
    if (!ios.has(name)) {
      errors.push(
        `${name}: missing <string>${name}.ttf</string> in ios/NovaMind/Info.plist (UIAppFonts)`,
      );
    }
  }
