/**
 * NovaHub authenticated purchase flow (iOS + US Android).
 *
 ...
 * against the novahub e2e test harness, which returns a deterministic
 ...
 * env vars (see e2e/helpers/novahubTestApi.ts):
 *   E2E_NOVAHUB_BASE_URL, E2E_API_KEY, E2E_BUYER_EMAIL, E2E_BUYER_PASSWORD,
 *   E2E_NOVAHUB_PAL_ID
 */
...
const getAppBundleId = (): string =>
  driver.isAndroid ? 'com.novamind.e2e' : 'ai.novamind';

describe('NovaHub authenticated purchase', () => {
  ...
  it('completes checkout and flips Buy to Download', async () => {
    ...
    await purchasePage.openPalDetail(novahubTestConfig.palId);
    ...
    await purchasePage.signInAndStartCheckout(
      novahubTestConfig.email,
      novahubTestConfig.password,
    );
