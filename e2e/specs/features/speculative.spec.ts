 ...
 *    `draft_tokens === 0` (NovaMind resolved to OFF and never sent spec_type).
 ...
 * element is present (draft_tokens > 0); off-safety asserts it is ABSENT (NovaMind resolved
...
describe('Speculative Decoding / MTP draft model', () => {
  ...
  it('off-safety: non-MTP model + speculative on loads, outputs, no native error', async () => {
    // NovaMind must resolve this to OFF (target is non-MTP, no valid draft) and
  ...
  it('crash-safety: a width-mismatched paired draft does not abort the process', async () => {
    ...
    // NovaMind must decline paired (unknown/mismatched width => not paired) and
    ...
    // on (from the off-safety test) -- NovaMind's width gate resolved it to off (no valid paired
