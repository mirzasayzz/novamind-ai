/**
 * Page object for the NovaHub premium-pal purchase flow:
...
export class PalPurchasePage extends BasePage {
  private palCard(palId: string): string {
    return byTestId(`novahub-pal-card-${palId}`);
  }
  ...
  private static readonly APP_FG =
    /com\.novamind\.e2e\/com\.novamind\.MainActivity/;

  /**
   * Once the checkout Custom Tab takes the foreground, dismiss it with hardware
   * BACK and wait for NovaMind to return. Best-effort: the e2e harness checkout
