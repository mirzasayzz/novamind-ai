/**
 * NovaHub test-support API client (e2e only).
 *
 ...
 *
 * Required env (set in e2e/.env or the CI env). Names match the novahub
 * test-harness contract:
 *   E2E_NOVAHUB_BASE_URL - test server base, e.g. http://192.168.0.92:3010
 ...
 *   E2E_NOVAHUB_PAL_ID   - premium fixture pal id (defaults to the seeded pal)
 */

export const novahubTestConfig = {
  baseUrl: process.env.E2E_NOVAHUB_BASE_URL || 'http://192.168.0.92:3010',
  testKey: process.env.E2E_API_KEY || '',
  email: process.env.E2E_BUYER_EMAIL || '',
  password: process.env.E2E_BUYER_PASSWORD || '',
  palId:
    process.env.E2E_NOVAHUB_PAL_ID ||
...
async function post(path: string, body: Record<string, unknown>): Promise<void> {
  if (!novahubTestConfig.testKey) {
    throw new Error(
      'E2E_API_KEY is not set — cannot call NovaHub test endpoints',
    );
  }

  const url = `${novahubTestConfig.baseUrl}${path}`;
  const response = await fetch(url, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-E2E-Key': novahubTestConfig.testKey,
    },
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `NovaHub test API ${path} failed: ${response.status} ${response.statusText} ${text}`,
    );
  }
}
...
export async function ensureTestUser(
  email: string = novahubTestConfig.email,
  password: string = novahubTestConfig.password,
): Promise<void> {
...
export async function resetPalOwnership(
  palId: string = novahubTestConfig.palId,
  userEmail: string = novahubTestConfig.email,
): Promise<void> {
