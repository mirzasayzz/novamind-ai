...
Drives the in-app **BenchmarkRunnerScreen** via deep link (`novamind://e2e/benchmark`) — no WDIO required for ad-hoc runs. Three tiers gated by `BENCH_TIER`:
...
# Installs as com.novamind.e2e and coexists side-by-side with the
# prod install (com.novamind). E2E_BUILD=true is set automatically
# so the automation bridge (src/__automation__/) ships in this APK.
```

**Flavor.** E2E targets the `e2e` flavor (`com.novamind.e2e`, debuggable),
...
client entries for both `com.novamind` and `com.novamind.e2e`. If the
...
| `E2E_APP_PATH` | `../ios/build/.../NovaMind.app` | `../android/.../app-e2e-releaseE2e.apk` | Path to built app |
...
APK is non-debuggable, so `adb shell run-as com.novamind` and
`adb push` into `/data/data/com.novamind/files/…` will not work.
...
```
/data/data/com.novamind.e2e/files/models/hf/<author>/<repo>/<filename>.gguf
```

Push each GGUF once:

```bash
adb shell run-as com.novamind.e2e mkdir -p \
...
adb shell run-as com.novamind.e2e sh -c \
