...
class MockPalStore {
  ...
  getDownloadedNovaHubPals = jest.fn(() =>
    this.pals.filter(p => p.source === 'novahub'),
  );
  searchNovaHubPals = jest.fn(async () => {});
  loadUserLibrary = jest.fn(async () => {});
  loadUserCreatedPals = jest.fn(async () => {});

  // NovaHub-related methods
  isNovaHubPalDownloaded = jest.fn(() => false);
  downloadNovaHubPal = jest.fn(async () => {});
}
